...
def test_parse_multiple_candidates_HLA_DRA_01_01_DRB1_01_01_G86Y_mutant():
    parser = Parser()
    results = parser.parse_multiple_candidates("HLA-DRA*01:01/DRB1*01:01 G86Y mutant")
    expected_mutation = Mutation.get(pos=86, aa_original="G", aa_mutant="Y")
    expected_alpha = Allele.get("HLA", "DRA", "01", "01")
    expected_beta = Allele.get("HLA", "DRB1", "01", "01", mutations=(expected_mutation,))
    expected_result = Pair.get(expected_alpha, expected_beta)
...
def test_parse_allele_or_gene_candidates_DRB5_0108N():
    ...
    results = parser.parse_allele_or_gene_candidates(
        species=Species.get("HLA"), str_after_species=seq
    )
...
def test_parse_allele_or_gene_candidates_A2():
    parser = Parser()
    results = parser.parse_allele_or_gene_candidates(
        species=Species.get("HLA"), str_after_species="A2"
    )
...
def test_parse_parse_allele_or_gene_candidates_BF2():
    parser = Parser()
    results = parser.parse_allele_or_gene_candidates(
        species=Species.get("chicken"), str_after_species="BF2"
    )
    assert len(results) == 1
    result = results[0]
    assert result is not None
...
def test_parse_H2_IAb_I67F_R70Q_T71K_mutant():
    ...
    for fn in parse_fns:
        result = fn(s)
        assert result is not None
        assert type(result) in (Pair, Allele), f"Wrong result type: {result}"
...
def test_candidates_from_ambiguous_class2_DPA10105_DPB110001():
    ...
    eq_(
        results[0],
        Pair.get(Allele.get("HLA", "DPA1", "01", "05"), Allele.get("HLA", "DPB1", "100", "01")),
    )
...
def test_candidates_from_ambiguous_mouse_class2_IAb():
    ...
    results = parser.parse_multiple_candidates(s)
    eq_(len(results), 2)
    class2 = Pair.get(Allele.get("H2", "AA", "b"), Allele.get("H2", "AB", "b"))
...
def test_parse_allele_with_gene_MICA_038():
    parser = Parser()
    allele = parser.parse_allele_with_gene(gene=Gene.get("HLA", "MICA"), str_after_gene="038")
...
def test_parse_allele_B_27_215N():
    parser = Parser()
    allele = parser.parse_allele_with_gene(gene=Gene.get("HLA", "B"), str_after_gene="27:215N")
