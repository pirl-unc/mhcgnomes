...
def test_parse_trout_gene_Onmy_DAA1():
    """DAA1 is now a real gene on Actinopterygii, not just an alias for DAA."""
...
def test_parse_common_carp_numbered_loci_and_aliases():
    ...
    examples = [
        ...
        # DAB1-3 are now independent genes, not aliases for DAB
        ("Cyca-DAB1", Gene.get("Cyca", "DAB1")),
        ("Cyca-DAB2", Gene.get("Cyca", "DAB2")),
        ("Cyca-DAB3", Gene.get("Cyca", "DAB3")),
        # DAB4 is still aliased to DAB (only DAB1-3 on Actinopterygii)
...
def test_parse_common_carp_alias_alleles():
    examples = [
        ...
        # DAB1/3 are now real genes — alleles resolve to them
        ("Cyca-DAB1*01", Allele.get("Cyca", "DAB1", "01")),
        ("Cyca-DAB3*01", Allele.get("Cyca", "DAB3", "01")),
        # DAB4 still aliases to DAB
