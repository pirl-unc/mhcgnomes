#!/usr/bin/env bash
# Run the mhcgnomes test suite with a memory- and CPU-aware pytest-xdist
...
if python -c "import xdist" 2>/dev/null; then
    ...
    log "workers=${WORKERS} → exec pytest -n ${WORKERS} --cov=mhcgnomes/ --cov-report=term-missing tests $*"
else
